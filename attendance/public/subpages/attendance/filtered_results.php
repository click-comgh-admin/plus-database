<?php
    ...
    <?php } else {
        $by_user_meeting_day = @(isset($_COOKIE[$_by_user_meeting_day])) ? $_COOKIE[$_by_user_meeting_day] : 0;
        $this_user_meeting_day = @(isset($_COOKIE[$_user_meeting_day])) ? $_COOKIE[$_user_meeting_day] : "all"; 
        $by_user_meeting_day = ($by_user_meeting_day == 0) ? 0: 1;?>
        <script> __set_cookies__("<?= $_by_user_meeting_day; ?>", "<?= (strtolower($this_user_meeting_day) == "all") ? 0: 1; ?>", $time = 2); </script> <?php
    }
