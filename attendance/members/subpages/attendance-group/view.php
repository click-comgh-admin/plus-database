<?php
    ...
    $groups = $ccApi->fetch_from($get_string=$get_string, function($groups) {
...
<div class="row justify-content-center">
    <div class="col-sm-12 col-md-5 col-lg-4">
        <div class="main-card mb-3 card shadow">
            <div class="card-body">
                ...
                <?php require_once Current_BaseDir."/forms/attendance-group/group-create_form.php"; ?>
            </div>
        </div>
    </div>
    <div class="col-sm-12 col-md-7 col-lg-8">
        <div class="main-card mb-3 card shadow">
            <div class="card-body">
                <h5 class="card-title"><?= ((int)AccountInfoType === 10) ? "Departments": "Groups"; ?></h5>
                                ...
                                foreach ($groups as $key => $group) { 
                                    ...
                                    <tr>
                                        ...
                                        <td style="white-space: nowrap;">
                                            ...
                                            <br><a href="<?= CLIENT_BASE_URL; ?>attendance/settings/clocking-agents-group?group-id=<?= $group['pdmg_id']; ?>&view-type=2"
                                                class="btn btn-sm m-1 btn-info" type="button" target="_blank">
                                                Assign Agent(s)
                                            </a>
                                        </td>
                                        <td style="white-space: nowrap;" class="text-center">
                                            <?= edit_btn($group, $account_id, $user_id, "Edit ". ((int)AccountInfoType === 10) ? "Department": "Group", "ml-2 p-1 btn btn-sm btn-warning shadow", $tf=$tf, $type = "md"); ?>
                                            <?php 
                                ...
                                foreach ($groups as $key => $group) { 
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <?= edit_btn($group, $account_id, $user_id, "Edit This ".((int)AccountInfoType === 10) ? "Department": "Group", "btn btn-sm m-1 btn-warning shadow", $tf=$tf, $type = "sm"); ?>
                                            <?php 
                                                $group_id = $group['pdmg_id']; 
...
<?php
    function show_subgroups($group, $account_id, $ccApi) { ?>
        <a href="#" data-toggle="modal" open-modal--id="subgroups-<?= $group['pdmg_id']; ?>"
            open-modal--title="<?= $group['pdmg_group']; ?> <?= ((int)AccountInfoType === 10) ? "Units": "Sub-Groups"; ?>" open-modal--html='<?php 
                $group_id = $group['pdmg_id']; 
    ...
    function edit_btn($group, $account_id, $user_id, $confirm_msg, $class, $tf, $type = "md") { ?>
        <button class="<?= $class; ?>" type="button" data-toggle="modal"
            ...
            open-modal--title="Edit Group" open-modal--html='<?php 
                $group_id = $group['pdmg_id']; 
                $group_name = $group['pdmg_group']; 
                require Current_BaseDir."/forms/attendance-group/group-edit_form.php"; 
            ?>' open-modal--buttons='
                <button class="btn btn-secondary" type="button" data-dismiss="modal">Close</button>
            '>
    ...
    function show_info_btn($group, $created_by, $TimeAgo, $updated_by, $updated_TimeAgo, $class) { ?>
        <button class="<?= $class; ?>" type="button" data-toggle="modal"
            open-modal--id="info_group-<?= $group['pdmg_id']; ?>" open-modal--title="Group Info"
            open-modal--html='<?php 
    ...
    function delete_btn($group_id, $account_id, $user_id, $class) { ?>
        <button class="<?= $class; ?>"
            ...
            btn-submit-post--url="<?= API_BASE_URL."attendance/clients/members/group/delete"; ?>">
