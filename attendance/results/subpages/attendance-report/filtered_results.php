<?php
    ...
    <?php } else {
        ...
        $by_gender = ($by_gender == 0) ? 0: 1;?>
        <script> __set_cookies__("<?= $_by_gender; ?>", "<?= (strtolower($this_gender) == "all") ? 0: 1; ?>", $time = 2); </script> <?php
    }
    ...
    <?php } else {
        $by_attendance_status_id = @(isset($_GET[$_by_attendance_status_id])) ? $_GET[$_by_attendance_status_id] : 0;
        $this_attendance_status_id = @(isset($_GET[$_attendance_status_id])) ? $_GET[$_attendance_status_id] : "all"; 
        $by_attendance_status_id = ($by_attendance_status_id == 0) ? 0: 1;?>
        <script> __set_cookies__("<?= $_by_attendance_status_id; ?>", "<?= (strtolower($this_attendance_status_id) == "all") ? 0: 1; ?>", $time = 2); </script> <?php
    }
    
    $_by_user_meeting_day_id = "by_user_meeting_day_id";
    $_user_meeting_day_id = "user_meeting_day";
    if (isset($_GET[$_user_meeting_day_id])) { 
        $getMid = ($_GET[$_user_meeting_day_id]);
    ...
    <?php } else {
        $by_user_meeting_day_id = @(isset($_GET[$_by_user_meeting_day_id])) ? $_GET[$_by_user_meeting_day_id] : 0;
        $this_user_meeting_day_id = @(isset($_GET[$_user_meeting_day_id])) ? $_GET[$_user_meeting_day_id] : "all"; 
        $by_user_meeting_day_id = ($by_user_meeting_day_id == 0) ? 0: 1;?>
        <script> __set_cookies__("<?= $_by_user_meeting_day_id; ?>", "<?= (strtolower($this_user_meeting_day_id) == "all") ? 0: 1; ?>", $time = 2); </script> <?php
    }
?>